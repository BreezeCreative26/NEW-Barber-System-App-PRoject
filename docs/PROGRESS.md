...
## WIP — shop page phase 2: SEO, verified reviews, R2 photos (2026-09-15, commit db59966, GATE NOT FINISHED)

- **Built:** `docs/SHOP-PAGE-PLAN.md`; migration `0017_reviews_media` (applied locally); `src/server/presence.ts` (reviews, media helpers, `shopPageHead` JSON-LD `HairSalon` + OG/Twitter/canonical, `headData`); `/<slug>` now server-renders the SEO head and 404s when `published=0` (public `/page` API too); `/robots.txt`, `/sitemap.xml` (published online shops), `/media/:id` (R2, immutable cache); `wrangler.jsonc` R2 binding `MEDIA`; `imageRef` schema accepts https or `/media|/static` paths (cover, gallery, staff photo); reviews API — public `POST /manage/:token/review`, account `POST …/bookings/:id/review`, `/manage` + `/me` return `review`/`can_review`; owner `GET /reviews`, `POST /reviews/:id/status|reply` (OWNER/MANAGER), `GET/POST /media`, `DELETE /media/:id`; `review_request` outbox template queued on COMPLETED (status change + checkout); client `Reviews.tsx` (Stars, ReviewCard on manage + /me history, PublicReviews section + hero rating on ShopPage); demo seed: CC/PD photos in `public/static/demo/` (CREDITS.md), 14 reviews (one hidden, two replies); contract lists updated; `shop-page.spec.ts` gallery assertion adjusted (seed now has 4 photos); url inputs switched to `type=text inputMode=url` so `/static/...` values pass browser validation.
- **Verified:** tsc clean, guardrails PASS, vitest 26, build OK, `/demo` head shows title "Demo Barbershop · Barbers in London", og:image cover, aggregateRating 4.7/13; `shop-page`, `public`, `sandbox`, `customer-account` specs pass (37). Evidence `docs/evidence/v12-shop-{hero,reviews,team}-1440.png`.
- **NOT done:** full Playwright gate (run died with sandbox 500s / connection lost — rerun `npm run test:e2e` alone after `pm2 restart barbershop-preview`); `npm run test:db` (D1 invariants) rerun; **no owner UI yet** for Settings → Reviews (hide/show/reply) or photo Upload buttons in Shop page / barber editors — API exists, panels not built; no dedicated `tests/shop-presence.spec.ts` (SEO head, robots/sitemap, hidden 404, review submit/moderation, media upload sniffing/scrub); README/CUSTOMER-PLAN not updated for this slice; `public/static/brand/og-default.svg` referenced by the head does not exist yet (add or point at ollo-mark.svg).
