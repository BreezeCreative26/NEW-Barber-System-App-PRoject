...
## Checkout is the only ceremony; calendar drag-to-move and per-day hours (2026-09-17)
- **Removed the check-in → start-service → pay ceremony.** Any live visit (CONFIRMED / CHECKED_IN / IN_SERVICE) has one primary action: **Checkout · £X**. The checkout screen is method-first — amount, tip, *How are they paying?* (Card / Cash / Transfer / Voucher), one button that completes the visit. With Stripe live the Card tile reads "Reader or QR" and the button becomes *Charge by card* → QR / text / email / reader; "Record card taken elsewhere" stays for unconnected machines. *Mark done without payment* moved to the ⋯ menu. Server: `/checkout` accepts CONFIRMED (advances to IN_SERVICE for the ledger trigger); `/status` allows CONFIRMED→COMPLETED directly. Statuses CHECKED_IN / IN_SERVICE remain valid for API clients but the UI no longer asks for them.
- **Calendar**: drag a confirmed appointment to another slot or barber → confirm → `/reschedule` (managers, or barbers when till_access=ALL). Drop ghost shows the landing slot; blocked slots refuse. Barber column header now shows that day's shift as a chip (**09:30–18:00**, "Off", "Day off"; highlighted when overridden) — click to edit **that day's hours** for that barber (existing dated-override editor, copy rewritten for the day-by-day use). Weekly hours per barber remain in Team → barber → Hours; shop opening hours per weekday in Settings.
- Tests updated (payments guards, sandbox lifecycle, payouts browser flow). Deposit copy in the appointment panel / group booking no longer says "not collected".
...
## Deposits via Stripe Checkout — done (superseded in part by the Connect platform above)
