...
Version 1.5 · 2026-09-14. **Latest E1 verification: build/typecheck, 42 unit/route/domain, direct local D1 invariants and 68 browser/API cases passed with unchanged source; zero failed/skipped/flaky and zero reported dependency vulnerabilities.** Report `test-results/runs/1789394389773-60993/results.json`, 13:59:49.778 UTC, 132.329s. Eight added cases cover native-button roving slot focus, arrow/Home/End traversal without mutation, activation/focus restoration, filter/colour/occupancy continuity, unavailable states and populated 320/390/768/1024/1440 views with axe, short/long cards and CSS 200% zoom. Final-source screenshots inspected and preserved as `docs/evidence/e1-*`; HTTPS preview workflow passed without page errors. Physical devices/screen-reader review, many-barber scale and user acceptance remain pending. No authentication/payment gate is passed by E1.

Historical baseline — **Latest connected-calendar verification: 42 unit/route/domain tests, direct D1 invariants and 60 browser/API cases passed on unchanged source.** Added complete >500-record day/impact reads, cursor ties and tenant/query validation, timetable click/create/reload/move/cancel, preserved multi-row pricing drafts, explicit dialog discard, response recovery and 390/1440 timetable checks. Existing five-width/fixture regressions retained. See PROGRESS for final 13:24 UTC report and remaining gaps. T-ENH contracts below remain broader than this delivery: no full role/provider/device acceptance is claimed.

Prior WP-LOCAL-02 evidence: D-012/D-013 retain the matte styling and local-only development. Current evidence: 42 unit/route/domain tests, direct local D1 invariants and 54 browser/API cases, all passing in the unchanged-source final regression. All 21 mutation routes have Origin/session/invalid-input coverage. New tests cover add-on/service links, exact aggregate durations, price/duration overrides, disabled coverage, dated replacement shifts, snapshot preservation and selection/conflict recovery alongside the complete earlier regression. The full migration chain also passed on an empty local D1 database. See PROGRESS for exact report and screenshots. The production acceptance catalogue remains incomplete: local workspace tests do not prove managed identity, checkout holds, payments, notifications or PWA acceptance.
