...
3. Test changed flows, persistence, failure recovery, keyboard and responsive layouts. Include phone/tablet/desktop, landscape, short screens, long content and zoom where relevant. Inspect actual browser images. Representative widths are not certification of every device.
   **Fast loop / slow gate (2026-09-14):** while iterating run only the affected file(s) — `npm run test:area -- tests/<area>.spec.ts [-g "name"]` — never the whole suite between fixes. Run the full gate (`npm test`: typecheck → vitest → D1 invariants → all Playwright) **once** at the end of the task, before the commit. Screenshots/axe evidence and README/PROGRESS updates are batched at handoff, not per slice. Browser tests that need a seeded shop use `openFixtureShop(page)` from `tests/fixture.ts` (private copy of the demo seed, parallel-safe); only the demo-account test itself may `rebuild` the shared demo. Prefer `data-testid` hooks over visible text in new tests so redesigns don't break them.
...
- **Design system:** every UI change follows `docs/DESIGN.md` — compose from `ui.tsx` primitives and `design.css` tokens, Lucide icons only, one primary action per surface, `data-testid` on new controls, phone-first for staff surfaces and desktop-first for owner surfaces. `npm run test:design` and `npm run test:visual` must pass; refresh visual baselines only deliberately. New areas get a static mock in `docs/mockups/` and approval before build.
...
- Existing migrations, including two distinct `0002_*` files, remain intact. Next prefix 0011. Identity/customer ownership, finance, providers and device acceptance remain separate unfinished work.
