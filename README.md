...
- **Booking flow upgrades (new):** "Any barber, see all times" shortcut from the service step; "This visit is for someone else" puts the attendee on the visit while contact stays the booker's (shown everywhere the visit appears); **group bookings** for 2–4 people — together (a chair each) or back to back (one barber) — each visit its own record under every guard with a shared group id, partial failures reported honestly.
...
- **Shop page: search, reviews, photos.** `/<slug>` ships a server-rendered head (title "Shop · Barbers in Town", description, canonical, Open Graph, JSON-LD `HairSalon` with hours, catalogue, team and `aggregateRating`); `/robots.txt` + `/sitemap.xml`; hidden pages 404. **Verified reviews** — only the person who had a completed visit can rate (manage link or `/me`), once per booking; shown as "First L."; owners hide/show/reply in Settings → Reviews; a `review_request` message is queued to the outbox on completion. **Photos** — upload cover, gallery and barber portraits (R2, `/media/<id>`) from the editors, or paste https URLs. Plan: `docs/SHOP-PAGE-PLAN.md`.
- **Waiting list & offers:** fully booked days offer "Join the waitlist" (name, mobile, preferred part of day; upserts per phone/date/service; a *joined* message is recorded). Staff manage the list from the top-bar **Queue** chip → drawer: Offer a time (real free matches, soft hold for the shop's hold minutes), Book them in, Remove; offered messages are shown to copy. Customers accept/decline at `/offer/<token>`; accept creates the booking. Auto-offer re-offers freed slots (cancel/move/decline/expiry). Settings → **Waitlist & messages** edits templates, hold time, auto-offer and shows the **Outbox** (all messages recorded as *Not sent* — no provider). Procedure: `docs/WAITLIST-PLAN.md`.
...
| `/api/public/shops/:slug/group-availability`, `/group-bookings` | GET together / back-to-back options for 2–4 members on a day; POST save the party (201 all, 207 partial with `failed[]`, 409 none) |
...
| `/api/sandbox/waitlist`, `/waitlist/:id/status`, `/waitlist/:id/matches`, `/waitlist/:id/offer` | GET `status=ACTIVE|OPEN|OFFERED|BOOKED|CLOSED|EXPIRED` entries + counts + settings (barber-scoped); POST versioned status; GET free matching slots; POST offer `{staff_id,start_min,version}` |
| `/api/sandbox/shop/waitlist`, `/api/sandbox/notifications` | PUT auto-offer / hold minutes / templates (OWNER/MANAGER); GET outbox + templates |
| `/api/public/offer/:token`, `…/accept`, `…/decline` | Customer offer view; accept → booking + manage link; decline `{leave?}` |
| `/api/public/manage/:token/review`, `/api/public/shops/:slug/account/bookings/:id/review` | POST `{rating 1–5, body?}` once per COMPLETED visit within 60 days; `GET /manage/:token` and `/me` expose `review`/`can_review` |
| `/api/sandbox/reviews`, `/reviews/:id/status`, `/reviews/:id/reply` | GET all (barber-scoped); POST `{status PUBLISHED|HIDDEN, version}`; POST `{reply ≤400, version}` (OWNER/MANAGER) |
| `/api/sandbox/media`, `/media/:id`, public `/media/:id` | POST multipart upload (≤5 MB JPEG/PNG/WebP, sniffed); GET library; DELETE scrubs references; served immutable |
| `/<slug>`, `/robots.txt`, `/sitemap.xml` | Server-rendered SEO head (OG, Twitter, JSON-LD HairSalon); hidden pages 404; published online shops listed |
| `/api/public/shops/:slug/account/waitlist/:id/leave` | Signed-in customer leaves the list (`/me` lists `waiting[]`) |
